...
public class ViaLinkFlutterPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    /// 3.1.0+: 딥링크는 일반 진입 시 nil payload를 전달하므로, link 진입(map != nil) /
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "configure":
            ...
            // 3.1.0+: data == nil은 일반 진입을 의미한다. EventSink는 nil payload를 그대로 전달한다.
            ViaLinkSDK.shared.onDeepLink { [weak self] data in
                let map: [String: Any?]? = data?.toFlutterMap()
    ...
    public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        deepLinkSink = events
        // 3.1.0+: pendingDeepLink가 nil이어도 hasPendingDeepLink가 true면 일반 진입 콜백을 flush.
        if hasPendingDeepLink {
            events(pendingDeepLink as Any?)
            pendingDeepLink = nil
